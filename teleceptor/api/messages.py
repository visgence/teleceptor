...
class Messages:
    ...
    def GET(self, sensor_id=None):
        ...
        #         try:
        #             messages = getMessages(sensor_id, session)
        ...
        #         message_queues = getAllMessages(session)
    ...
    def POST(self):
        ...
        # with sessionScope() as session:
        #     try:
        #         msg = addMessage(data['sensor_id'], float(data['message']), data['duration'], session)
    ...
    def DELETE(self, sensor_id, message_id):
        ...
        # with sessionScope() as session:
        #     try:
        #         msg = deleteMessage(sensor_id, message_id, session)
        #     except NoResultFound as e:
        #         return_data['error'] = e.message
        #         status_code = "400"
        #     except ValueError as e:
    ...
    def PURGE(self, sensor_id, timeout):
        ...
        # with sessionScope() as session:
        #     try:
        #         rows_deleted = deleteAllMessages(sensor_id, timeout, session)
        #     except NoResultFound as e:
        #         return_data['error'] = e.message
        #         status_code = "400"
        #     except ValueError as e:
...
def getMessages(sensor_id, by_timestamp=False, unread_only=False):
    ...
    sensor_queue = MessageQueue.objects.filter(sensor=sensor_id).last()
    if not sensor_queue:
    ...
    if unread_only:
        if by_timestamp:
            ...
            messages = Message.objects.filter(Message.message_queue_id == sensor_queue.id,
...
def addMessage(sensor_id, message, duration, session):
    """
...
def deleteMessage(sensor_id, message_id, session):
    """deleteMessage"""
    # try:
    #     sensor = session.query(Sensor).filter_by(uuid=sensor_id).one()
    # except NoResultFound as e:
...
def deleteAllMessages(sensor_id, timeout, session):
    """deleteAllMessages"""
    # rows_deleted = None
    # try:
    #     sensor = session.query(Sensor).filter_by(uuid=sensor_id).one()

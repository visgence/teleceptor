...
class DataStreams(APIView):
    ...
    @staticmethod
    def createDatastream(datastream=None):
        """
        ...
        if datastream is not None:
            logging.debug("Creating datastream with options: %s", str(datastream.to_dict()))
            DataStream.objects.create(datastream)
        else:
...
def deleteDatastream(session, datastream_id):
    ...
    try:
        stream = DataStream.objects.delete(id=datastream_id)
...
def get_datastream(datastream_id, session=None):
    ...
    # stream = session.query(DataStream).filter_by(id=datastream_id)
    # return stream.to_dict()
...
def get_datastream_by_name(datastream_name, session=None):
    ...
    #     with sessionScope() as s:
    #         session = s
    # stream = session.query(DataStream).filter_by(name=datastream_name)
    # return stream.to_dict()
